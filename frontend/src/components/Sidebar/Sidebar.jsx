            ...
            <nav className="sidebar-nav">
                {menuItems.map((item, idx) => (
                    <NavLink
                        key={item.path}
                        to={item.path}
                        className={({ isActive }) => `nav-item stagger-${idx + 1} ${isActive ? 'active' : ''}`}
                    >
                        <span className="nav-icon">{item.icon}</span>
                        <span className="nav-name">{item.name}</span>
                        {item.name === 'INTELIGÊNCIA' && <span className="nav-badge shimmer-bg">L3</span>}
                    </NavLink>
                ))}
            </nav>
            ...
            <div className="sidebar-footer">
                <div className="user-persona-card">
                    ...
                    <div className="persona-info">
                        ...
                        <span className="persona-status text-forest-green">OFFICIAL ACCESS</span>
                    </div>
                </div>
            </div>
        </aside>
