  ...
  if (loading)
    return (
      <div className="flex h-[80vh] items-center justify-center">
        <div className="text-silver-mute font-medium tracking-[0.3em] uppercase animate-pulse">
          Sincronizando Estratégia...
        </div>
      </div>
    );
      ...
      <header className="flex justify-between items-end opacity-0 animate-entrance">
        <div className="space-y-1">
          ...
          <p className="text-mute text-sm">
            Gestão de suprimentos e otimização de fluxo MA3.
          </p>
        </div>
          ...
          {lists.length === 0 ? (
            <div className="card-panel p-20 text-center border-dashed opacity-0 animate-entrance">
              <p className="text-mute text-sm italic">
                Nenhuma lista ativa para esta estratégia.
              </p>
            </div>
          ) : (
            lists.map((list, idx) => (
              <div
                key={list.id}
                className="card-panel p-10 opacity-0 animate-entrance group transition-all duration-700"
              >
                <div className="flex justify-between items-center mb-10">
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      <h3 className="text-xl font-bold text-charcoal tracking-tight group-hover:text-forest-green transition-colors">
                        {list.name || "Nova Lista"}
                      </h3>
                      <p className="text-[10px] text-mute uppercase tracking-[0.2em] font-bold">
                        {list.items?.length || 0} Itens Vinculados
                      </p>
                    </div>
                  </div>
                ...
                <div className="space-y-4">
                  {list.items?.map((item) => (
                    <div
                      key={item.id}
                      className="flex justify-between items-center p-6 bg-surface border border-border-light rounded-3xl hover:bg-white transition-all"
                    >
                      <div className="flex items-center gap-4">
                        ...
                        />
                        <span
                          className={`text-sm font-medium ${item.purchased ? "text-mute line-through opacity-50" : "text-charcoal"}`}
                        >
                          {item.name}
                        </span>
                      </div>
        ...
        <div className="space-y-10">
          {processing ? (
            <div className="card-panel p-12 text-center animate-pulse">
              ...
              <p className="text-[10px] text-charcoal font-bold tracking-[0.3em] uppercase mb-2">
                Neural Scan...
              </p>
              <p className="text-xs text-mute">
                Decodificando topologia fiscal...
              </p>
            </div>
          ) : result ? (
            <div className="card-panel p-10 border-forest-green/20 bg-emerald-surface opacity-0 animate-entrance">
              ...
              <div className="space-y-8">
                <div className="p-6 bg-white/50 rounded-3xl border border-border-light">
                  <p className="text-[10px] text-mute uppercase tracking-[0.2em] font-bold mb-3">
                    Estabelecimento
                  </p>
                  <p className="text-lg text-charcoal font-bold">
                    {(result.receipt || result).store || "Identificado"}
                  </p>
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="p-6 bg-white/50 rounded-3xl border border-border-light">
                    <p className="text-[10px] text-mute uppercase tracking-[0.2em] font-bold mb-2">
                      Total
                    </p>
                    <p className="text-xl font-bold text-charcoal tracking-tighter">
                      {fmt((result.receipt || result).total)}
                    </p>
                  </div>
                  <div className="p-6 bg-white/50 rounded-3xl border border-border-light">
                    <p className="text-[10px] text-mute uppercase tracking-[0.2em] font-bold mb-2">
                    ...
                    <p className="text-xl font-bold text-forest-green tracking-tighter">
                      {((result.receipt || result).impact_pct || 0).toFixed(2)}%
                    </p>
                  </div>
                </div>
                <div className="space-y-4">
                  ...
                  <div className="p-4 bg-white/80 rounded-2xl border border-forest-green/10">
                    <p className="text-xs text-forest-green leading-relaxed font-medium">
                      "
                      {(result.insight || result).interpretacao ||
            ...
            <div className="card-panel p-12 opacity-0 animate-entrance text-center group transition-all duration-700">
              ...
              <h4 className="text-charcoal font-bold mb-3 tracking-tight">
                Scanner de Recibos
              </h4>
              <p className="text-xs text-mute font-medium leading-relaxed">
                Arraste seu cupom fiscal para sincronizar gastos com o Kernel em
                tempo real.
              </p>
            </div>
          )}
