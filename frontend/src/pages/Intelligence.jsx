...
const Intelligence = () => {
  ...
  const weightMap = weights
    ? [
        ...
        {
          label: "MA3 / Frequência (w2)",
          weight: Math.round(weights.w2 * 100),
        },
        {
          label: "Urgência Categórica (w3)",
          weight: Math.round(weights.w3 * 100),
        },
        {
          label: "Conflito de Metas (w4)",
          weight: Math.round(weights.w4 * 100),
        },
        {
          label: "Confiança do Kernel (w5)",
          weight: Math.round(weights.w5 * 100),
        },
      ]
    : [];
    ...
    <div className="space-y-16 max-w-7xl mx-auto pb-20 p-8">
      <header className="space-y-1 opacity-0 animate-entrance">
        <h1 className="text-charcoal text-2xl font-bold tracking-tight">
          Intelligence Engine v8.1
        </h1>
        <p className="text-mute text-sm">
          Transparência analítica e auditoria de consciência financeira.
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 card-panel p-12 opacity-0 animate-entrance group transition-all duration-700">
          <div className="relative z-10">
            <h3 className="text-[10px] font-bold tracking-[0.3em] text-forest-green mb-12 uppercase">
              Persona Audit
            </h3>
            <div className="flex flex-col md:flex-row items-center gap-14">
              <div className="w-36 h-36 rounded-full card-panel flex items-center justify-center text-6xl bg-emerald-surface">
                {persona.detectedPersona === "C"
                  ? "⚔️"
                  : persona.detectedPersona === "A"
                    ? "🛡️"
                    : "⚖️"}
              </div>
              <div className="flex-1 space-y-8">
                ...
                <div className="grid grid-cols-2 gap-8">
                  <div className="p-6 bg-surface border border-border-light rounded-3xl">
                    <span className="text-[10px] text-mute block uppercase tracking-[0.2em] mb-2 font-bold">
                      Impacto Permitido
                    </span>
                    <span className="text-forest-green font-bold text-xl tracking-tight">
                      {persona.maxImpactPermitted}%
                    </span>
                  </div>
                  <div className="p-6 bg-surface border border-border-light rounded-3xl">
                    <span className="text-[10px] text-mute block uppercase tracking-[0.2em] mb-2 font-bold">
                    ...
                    <span className="text-charcoal font-bold text-xl tracking-tight">
                      {persona.recommendedSuavidade >= 4
                        ? "MÁXIMA"
                        : persona.recommendedSuavidade >= 3
                          ? "MÉDIA"
                          : "BAIXA"}
                    </span>
                  </div>
                ...
                <p className="text-sm text-mute leading-[1.8] font-medium italic">
                  "
                  {persona.cognitiveWarning ||
                    "O sistema opera em modo de vigilância ativa para proteção do seu patrimônio."}
                  "
                </p>
              </div>
            </div>
        ...
        <div className="card-panel p-12 opacity-0 animate-entrance stagger-1">
          <h3 className="text-[10px] font-bold tracking-[0.3em] text-charcoal mb-10 uppercase">
            Pesos Neurais
          </h3>
      ...
      <div className="card-panel p-12 opacity-0 animate-entrance stagger-2">
        <div className="flex justify-between items-center mb-12">
          <h3 className="text-[10px] font-bold tracking-[0.3em] text-charcoal uppercase">
            Fluxo Natural (PFS 4D §8)
          </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
          <div className="space-y-8">
            <p className="text-sm text-mute font-medium leading-[1.8]">
              O motor anti-ruído garante que você receba apenas os insights mais
              críticos, respeitando intervalos cognitivos para decisões precisas
              e filtragem estratégica.
            </p>
          ...
          <div className="grid grid-cols-2 gap-8">
            <div className="p-8 bg-surface border border-border-light rounded-[32px] text-center">
              <span className="text-[10px] text-mute block mb-3 uppercase tracking-[0.3em] font-bold">
                Cooldown
              </span>
              <span className="text-4xl font-bold text-charcoal tracking-tighter">
                00:00:00
              </span>
            </div>
            <div className="p-8 bg-surface border border-border-light rounded-[32px] text-center">
