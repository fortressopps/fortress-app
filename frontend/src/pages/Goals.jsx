    ...
    <div className="space-y-16 max-w-7xl mx-auto pb-20 p-8">
      <div className="flex justify-between items-center opacity-0 animate-entrance">
        <header className="space-y-1">
          ...
          <p className="text-mute text-sm">
            Diretrizes de crescimento e travas de segurança patrimonial.
          </p>
        </header>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        {goals.map((goal, idx) => (
          <div
            key={goal.id}
            className="card-panel p-10 group opacity-0 animate-entrance"
          >
            <div className="relative z-10 space-y-10">
              <div className="flex justify-between items-start">
                <div className="space-y-3">
                  <h3 className="font-bold text-xl text-charcoal tracking-tight group-hover:text-forest-green transition-colors">
                    {goal.name}
                  </h3>
              ...
              <div className="space-y-8">
                <div className="flex justify-between items-end">
                  <div className="space-y-1">
                    <p className="text-[10px] text-mute font-bold uppercase tracking-[0.3em]">
                      Target
                    </p>
                    <p className="font-bold text-charcoal text-3xl tracking-tighter">
                      {fmt(goal.value)}
                    </p>
                  </div>
                  <div className="text-right">
                    <span
                      className={`text-[10px] font-bold uppercase tracking-[0.2em] px-3 py-1 rounded-full ${goal.analysis?.atRisk ? "bg-red-100 text-red-600 border border-red-200" : "bg-emerald-surface text-forest-green border border-emerald-primary/20"}`}
                    >
                ...
                {goal.analysis?.deviation && (
                  <div className="pt-8 flex justify-between items-center border-t border-border-light">
                    <span className="text-[10px] text-mute uppercase tracking-[0.2em] font-bold">
                      Desvio Estimado
                    </span>
                    <span
                      className={`text-xs font-mono font-bold ${goal.analysis.deviation.status === "over" ? "text-red-500" : "text-forest-green"}`}
                    >
                      {goal.analysis.deviation.deviation_pct > 0 ? "+" : ""}
                      {goal.analysis.deviation.deviation_pct}%
                    </span>
                  </div>
                )}
              </div>
        ...
        {goals.length === 0 && (
          <div className="col-span-full py-24 text-center card-panel border-dashed opacity-0 animate-entrance">
            <p className="text-mute text-sm italic">
              Nenhuma diretriz estratégica definida para este ciclo.
            </p>
          </div>
        )}
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-charcoal/40 backdrop-blur-sm flex items-center justify-center p-8 z-[1000] animate-entrance">
          <div className="card-panel p-16 w-full max-w-xl shadow-2xl">
            <h2 className="text-3xl font-bold text-charcoal tracking-tight mb-10">
              Provisionar Diretriz
            </h2>
            <form onSubmit={handleCreate} className="space-y-10">
              <div className="space-y-3">
                <label className="text-[10px] font-bold text-mute uppercase tracking-[0.3em] ml-2">
                  Nome da Operação
                </label>
                <input
                  required
              ...
              <div className="grid grid-cols-2 gap-8">
                ...
                <div className="space-y-3">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNewGoal({ ...newGoal, periodicity: e.target.value })
                    }
                  >
