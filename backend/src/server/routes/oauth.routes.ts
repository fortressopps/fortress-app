...
// Registra estratégias OAuth apenas quando credenciais estão configuradas (evita falha em testes)
...
oauthRoutes.get('/auth/google/callback', passport.authenticate('google', { failureRedirect: '/login' }), async (c) => {
  const user = (c.req.raw as any).user as { id: string };
  if (!user?.id) return c.redirect('/login');
  const token = signAccessToken(user.id);
  return c.redirect(`${process.env.FRONTEND_URL || 'http://localhost:5173'}/oauth-callback?token=${token}`);
});
