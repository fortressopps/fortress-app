...
export const rateLimiter = (req, res, next) => {
  if (!ENV.ENABLE_RATE_LIMITING) return next();
...
export const checkPermission = (req, capability) => {
  if (!ENV.FEATURE_FLAGS.enforceRBAC) return true;
...
export const authMiddleware = (req, res, next) => {
  ...
  try {
    if (!ENV.AUTH_JWT_PUBLIC_KEY) throw new AppError('Chave pública JWT ausente', 500, 'JWT_KEY_MISSING');
...
export const validateBody = (schemaKey, body) => {
  ...
  if (!schema) throw new AppError('Schema de validação não encontrado', 500, 'SCHEMA_NOT_FOUND');
...
export const repo = {
  insertList: async ({ userId, listName, isActive }) => {
    const { data, error } = await supabase
      ...
      .select()
      .single();
    if (error) throw new AppError('Erro ao criar lista de compras', 500, 'DB_INSERT_LIST_FAILED');
    return data;
  },
  insertItems: async items => {
    const { error } = await supabase.from('supermarket_items').insert(items);
    if (error) throw new AppError('Erro ao adicionar itens à lista', 500, 'DB_INSERT_ITEMS_FAILED');
  },
  fetchListWithItems: async listId => {
    ...
    const { data, error } = await supabase
      ...
      .eq('id', listId)
      .single();
    if (error) throw new AppError('Erro ao buscar lista', 500, 'DB_FETCH_LIST_FAILED');
  ...
  fetchListById: async ({ listId, userId }) => {
    const { data, error } = await supabase
      ...
      .eq('user_id', userId)
      .single();
    if (error && error.code !== 'PGRST116') throw new AppError('Erro ao buscar lista', 500, 'DB_FETCH_LIST_BY_ID_FAILED');
    return data;
  },
  fetchListsByUser: async ({ userId, from, to }) => {
    ...
    const { data, count, error } = await supabase
      ...
      .range(from, to);
    if (error) throw new AppError('Erro ao buscar listas de compras', 500, 'DB_FETCH_LISTS_BY_USER_FAILED');
  ...
  checkListOwnership: async ({ listId, userId }) => {
    const { data, error } = await supabase
      ...
      .eq('user_id', userId)
      .single();
    if (error && error.code !== 'PGRST116') throw new AppError('Erro ao verificar lista', 500, 'DB_CHECK_OWNERSHIP_FAILED');
    return Boolean(data);
  },
  updateListById: async (listId, updates) => {
    const { error } = await supabase.from('supermarket_lists').update(updates).eq('id', listId);
    if (error) throw new AppError('Erro ao atualizar lista', 500, 'DB_UPDATE_LIST_FAILED');
    // Invalidate cache
  ...
  deleteItemsByListId: async listId => {
    ...
    if (error) throw new AppError('Erro ao atualizar itens da lista', 500, 'DB_DELETE_ITEMS_FAILED');
    cache.del(cacheKey('list_with_items', { listId }));
  },
  insertItem: async item => {
    const { data, error } = await supabase.from('supermarket_items').insert([item]).select().single();
    if (error) throw new AppError('Erro ao adicionar item', 500, 'DB_INSERT_ITEM_FAILED');
  ...
  fetchItemWithOwnership: async (itemId, userId) => {
    const { data, error } = await supabase
      ...
      .single();
    if (error && error.code !== 'PGRST116') throw new AppError('Erro ao buscar item', 500, 'DB_FETCH_ITEM_FAILED');
    return data;
  },
  updateItemById: async (itemId, updates) => {
    ...
    if (error) throw new AppError('Erro ao atualizar item', 500, 'DB_UPDATE_ITEM_FAILED');
    if (data?.list_id) cache.del(cacheKey('list_with_items', { listId: data.list_id }));
    return data;
  },
  deleteItemById: async itemId => {
    const { data, error } = await supabase.from('supermarket_items').delete().eq('id', itemId).select().single();
    if (error) throw new AppError('Erro ao remover item', 500, 'DB_DELETE_ITEM_FAILED');
    if (data?.list_id) cache.del(cacheKey('list_with_items', { listId: data.list_id }));
  },
  deleteListById: async listId => {
    const { error } = await supabase.from('supermarket_lists').delete().eq('id', listId);
    if (error) throw new AppError('Erro ao excluir lista', 500, 'DB_DELETE_LIST_FAILED');
    cache.del(cacheKey('list_with_items', { listId }));
  },
  fetchAnalyticsBase: async userId => {
    ...
    if (completedErr) throw new AppError('Erro ao buscar dados analíticos', 500, 'DB_FETCH_ANALYTICS_FAILED');
...
export const updateUserListsCount = async (userId, delta) => {
  try {
    if (delta > 0) await supabase.rpc('increment_user_lists_count', { user_id: userId });
    else if (delta < 0) await supabase.rpc('decrement_user_lists_count', { user_id: userId });
  } catch (e) {
...
export const services = {
  ...
  getListService: async ({ userId, listId }) => {
    ...
    if (!list || list.user_id !== userId) throw new AppError('Lista de compras não encontrada', 404, 'LIST_NOT_FOUND');
    return list;
  },

  updateListService: async ({ userId, listId, payload }) => {
    const exists = await repo.checkListOwnership({ listId, userId });
    if (!exists) throw new AppError('Lista de compras não encontrada', 404, 'LIST_NOT_FOUND');

    const updates = {};
    if (payload.listName?.trim()) updates.list_name = payload.listName.trim();
    if (typeof payload.isActive === 'boolean') updates.is_active = payload.isActive;

    if (Object.keys(updates).length > 0) await repo.updateListById(listId, updates);
    ...
    if (Array.isArray(payload.items)) {
      ...
      if (itemsToInsert.length > 0) await repo.insertItems(itemsToInsert);
    }
  ...
  addItemService: async ({ userId, listId, payload }) => {
    ...
    if (!list) throw new AppError('Lista de compras não encontrada', 404, 'LIST_NOT_FOUND');
    if (!list.is_active) throw new AppError('Não é possível adicionar itens a uma lista finalizada', 400, 'LIST_INACTIVE');
  ...
  updateItemService: async ({ userId, listId, itemId, payload }) => {
    ...
    if (!itemRow) throw new AppError('Item não encontrado', 404, 'ITEM_NOT_FOUND');

    const updates = {};
    if (payload.name?.trim()) updates.name = payload.name.trim();
    if (payload.category?.trim()) updates.category = payload.category.trim();
    if (payload.quantity !== undefined) updates.quantity = Math.max(1, Number(payload.quantity) || 1);
    if (payload.estimatedPrice !== undefined) updates.estimated_price = Number(payload.estimatedPrice) || null;
    if (payload.actualPrice !== undefined) updates.actual_price = Number(payload.actualPrice) || null;
    if (payload.unit?.trim()) updates.unit = payload.unit.trim();
    if (payload.store?.trim()) updates.store = payload.store.trim();
  ...
  markItemPurchasedService: async ({ userId, listId, itemId, payload }) => {
    const itemRow = await repo.fetchItemWithOwnership(itemId, userId);
    if (!itemRow) throw new AppError('Item não encontrado', 404, 'ITEM_NOT_FOUND');
    if (itemRow.purchased) throw new AppError('Item já está marcado como comprado', 400, 'ITEM_ALREADY_PURCHASED');
    ...
    if (savings > 0) await updateUserSavings(userId, savings);
  ...
  completeListService: async ({ userId, listId }) => {
    const list = await repo.fetchListById({ listId, userId });
    if (!list) throw new AppError('Lista de compras não encontrada', 404, 'LIST_NOT_FOUND');
    if (!list.is_active) throw new AppError('Lista já está finalizada', 400, 'LIST_ALREADY_COMPLETED');
    ...
    if (summary.totalSavings > 0) await updateUserSavings(userId, summary.totalSavings);
  ...
  getAnalyticsService: async ({ userId }) => {
    ...
    const totalSavings = completedLists.reduce((sum, list) =>
      sum + ((list.supermarket_items || []).reduce((itemSum, item) =>
        itemSum + ((item.estimated_price || 0) - (item.actual_price || 0)), 0)), 0);

    const totalSpent = completedLists.reduce((sum, list) =>
      sum + ((list.supermarket_items || []).reduce((itemSum, item) =>
        itemSum + (item.actual_price || 0), 0)), 0);
    ...
    const monthlyTrend = (() => {
      ...
      completedLists.forEach(list => {
        ...
        if (!monthlyData[month]) monthlyData[month] = { savings: 0, lists: 0, spent: 0, items: 0 };
        const items = list.supermarket_items || [];
...
export const controllers = {
  createList: catchAsync(async (req, res) => {
    if (!checkPermission(req, 'canCreateList')) throw new AppError('Permissão negada', 403, 'RBAC_DENY');
  ...
  updateItem: catchAsync(async (req, res) => {
    if (!checkPermission(req, 'canEditItem')) throw new AppError('Permissão negada', 403, 'RBAC_DENY');
  ...
  deleteList: catchAsync(async (req, res) => {
    if (!checkPermission(req, 'canDeleteList')) throw new AppError('Permissão negada', 403, 'RBAC_DENY');
    const { id } = req.params;
