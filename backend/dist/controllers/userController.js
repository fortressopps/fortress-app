...
export const getDashboard = catchAsync(async (req, res, next) => {
    ...
    const [user, supermarketLists, activeLists, completedLists] = await Promise.all([
    ...
    const totalSupermarketSavings = completedLists.reduce((sum, list) => sum + (list.savings || 0), 0);
    const totalSupermarketSpent = completedLists.reduce((sum, list) => sum + (list.totalActual || 0), 0);
...
export const getMetrics = catchAsync(async (req, res, next) => {
    const userId = req.user.id;
    const [supermarketLists, completedLists] = await Promise.all([
        SupermarketList.find({ userId }),
        SupermarketList.find({ userId, isActive: false })
    ]);
    const metrics = {
...
const calculateRankProgress = (territory) => {
    ...
    if (!nextRank)
        return 100; // Máximo rank alcançado
    const previousRank = nextRank.threshold === 100 ? 0 :
        [100, 250, 500, 1000, 2500, 5000, 10000][[100, 250, 500, 1000, 2500, 5000, 10000].indexOf(nextRank.threshold) - 1];
