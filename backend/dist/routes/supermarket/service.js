...
export async function list({ page = 1, pageSize = 20, userId = null } = {}) {
    ...
    const table = "supermarket_lists";
    const res = await dbQuery(supabaseAdmin, (c) => c
        .from(table)
        .select("*", { count: "exact" })
        .range(from, to)
        .order("date", { ascending: false }), { retry: 2, timeout: 9000, audit: true });
